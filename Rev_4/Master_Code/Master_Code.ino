...
void i2c_Tx_Arduinos (int Address, int DataToBeSent, int DataSizeinBytes);      // For starting basic Transmission between Master and Slave
...
void loop() 
{
  ...
  if ((switchDisplay == 5) || (switchDisplay == 6))     // Activating the fucntion for Analog Stick
{                                                       // reception from Slave Arduino 
    if ((currentMillis - previousMillis_6) > debounceInterval)
    {
...
void pressBUT_2 ()        // Using a normal tactile push button
{
  ...
  if ((butState_2 == LOW) && (keyPressed_2 == true) && ((currentMillis - previousMillis_2) > debounceInterval))
  {
    ...
    receiveFromSlave_2 =  Wire.read();// << 8;
    //receiveFromSlave_2 = receiveFromSlave_2 | Wire.read();
